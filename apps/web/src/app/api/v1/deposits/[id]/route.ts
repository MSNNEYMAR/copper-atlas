 ...
 *
 * Returns full deposit properties as a GeoJSON Feature.
 * Used by the detail panel when a user clicks a deposit on the map.
 */
...
export async function GET(_request: NextRequest, { params }: { params: { id: string } }) {
  try {
    const depRes = await fetch(`${API}/deposits?id=eq.${params.id}&limit=1`, { headers: H });
    ...
    // Fetch country + classification in parallel
